...
do_join(State, Ref, ChatName) ->
	Pid = whereis(list_to_atom(State#cl_st.gui)),
	case maps:is_key(ChatName, State#cl_st.con_ch) of
		true -> Pid!{result, self(), Ref, err},
				{err, State};	   
    	...
    	true -> whereis(server)!{self(), Ref, leave, ChatName},
    			receive
    				{_, Ref, ack_leave} ->
		    			...
		    			Pid!{result, self(), Ref, ok},
		    			{ack_leave, Updated}
		    	end;
    	false -> Pid!{result, self(), Ref, err},
    			 {err, State}
    end.
...
do_new_nick(State, Ref, NewNick) ->
    ...
    case string:equal(State#cl_st.nick, NewNick) of
    	true -> GuiPid!{result, self(), Ref, err_same},
    			{err, State};
...
do_msg_send(State, Ref, ChatName, Message) ->
    ...
    receive
    	{_, Ref, ack_msg} -> GuiPid!{result, self(), Ref, {msg_sent, State#cl_st.nick}},
    						 {ack_msg, State}
    end.
