...
do_update_nick(State, ClientPID, NewNick) ->
    Updated = State#chat_st{registrations = maps:put(ClientPID, NewNick, State#chat_st.registrations)},
    Updated.
...
do_propegate_message(State, Ref, ClientPID, Message) ->
    ...
    RecPIDS = lists:delete(ClientPID, maps:keys(State#chat_st.registrations)),
	Nick = maps:get(ClientPID, State#chat_st.registrations),
    lists:foreach(fun(PID) -> PID!{request, self(), Ref, {incoming_msg, Nick, State#chat_st.name, Message}} end, RecPIDS),
    Updated = State#chat_st{history = lists:append(State#chat_st.history, [{Nick, Message}])},
	Updated.
